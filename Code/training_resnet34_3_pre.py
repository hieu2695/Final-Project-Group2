...
trainset = DataAug(x_train_over, y_train_over, transform = train_data_transform ,length=5*len(x_train_over))
...
path ="../Model/model_resnet34_3_pre.pt"
...
plt.figure()
plt.plot(inds.astype(np.uint8), val_losses, label = "Focal loss")
plt.plot(inds.astype(np.uint8), running_f1, label = "Macro F-1 score")
plt.xlabel("Epoch")
plt.ylabel("Magnitude")
plt.title("Learning curve of resenet34_3_pre with base learning_rate={:.4f}".format(LR))
plt.legend(loc='best')
plt.savefig("../Figure/learning_curve_resnet34_3_pre.pdf")
plt.show()